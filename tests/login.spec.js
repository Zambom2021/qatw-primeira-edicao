...
test('Deve acessar a conta do usuario com consutla de 2FA via Banco de Dados', async ({ page }) => {
  const loginPage = new LoginPage(page)
...
test('Deve acessar a conta do usuario com consulta de 2FA via Redis', async ({ page }) => {
  const loginPage = new LoginPage(page)
  const dashPage = new DashPage(page)
